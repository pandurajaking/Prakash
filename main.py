...
keyboard = dict(keyboard=[["/pro_txt"]], resize_keyboard=True)
...
async def account_login(bot: Client, m: Message):
    ...
    if user is not None and user not in sudo_users:
        ...
        editable = await m.reply_text(
            "Hello Bruh **I am Video Downloader Bot**. I can download videos from **text** file one by one.**\n\nLanguage** : Python**\nFramework** :Pyrogram\n\nSend **TXT** File {Name : Link}"
    ...
    editable = await m.reply_text(
        f"Total links found are **{len(links)}**\n\nSend From where you want to download. Initial index is **0**"
    )
    ...
    await m.reply_text("**Enter resolution (360/480/720)**")
        ...
        for i in range(arg, len(links)):
            ...
            try:
                ...
                if "pdf" in url1:
                    ...
                    if resolution == "360":
                        await send_video(bot, m, filename, cc, 360, thumbnail, dur, reply, start_time)
                    elif resolution == "480":
                        await send_video(bot, m, filename, cc, 480, thumbnail, dur, reply, start_time)
                    elif resolution == "720":
                        await send_video(bot, m, filename, cc, 720, thumbnail, dur, reply, start_time)
                    else:
                        await m.reply_text("Invalid resolution! Please enter 360, 480, or 720.")
                        return
...
async def send_video(bot, m, filename, cc, resolution, thumbnail, dur, reply, start_time):
    try:
